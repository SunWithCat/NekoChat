...
interface ChatMessageDao {
    ...
    // 仅在自动生成标题时更新标题 (不更新时间戳，时间戳应该无条件更新)
    @Query(
        "UPDATE conversations SET title = :title WHERE id = :conversationId AND isCustomTitle = 0"
    )
    suspend fun updateConversationTitleIfAuto(conversationId: Long, title: String)
